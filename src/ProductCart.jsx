...
import { defaultProductDetail } from "./productCatalog";
...
function ProductCart({
  ...
  const [selectedImage, setSelectedImage] = useState(defaultProductDetail.gallery[0]);
  const [selectedOption, setSelectedOption] = useState(defaultProductDetail.variant);
  const [quantity, setQuantity] = useState(1);
  const [customText, setCustomText] = useState("");
  const closeTimerRef = useRef(null);
  const product = selectedProduct ?? defaultProductDetail;
  const variantLabel = product.options.every((option) => /\d/.test(String(option))) ? "Size" : "Color";
  ...
  return (
    <main className="page-shell">
      <header className="topbar">
        <div className="topbar__inner">
          ...
          <div className="topbar__actions">
            ...
            <button className="icon-link icon-link--count" type="button" aria-label="Cart" onClick={onCartOpen}>
              <BagIcon />
              <span>({cartCount})</span>
            </button>
          </div>
      ...
      <nav className="subbar">
        <div className="subbar__inner">
          ...
          <div className="subbar__menu">
            ...
            <button className="subbar__menu-btn" type="button" onClick={() => navigateTo("/blog")}>BLOG</button>
                    ...
                    typeof item === "string" ? (
                      <a
                        ...
                        onClick={(event) => {
                          ...
                          else if (item === "Refund policy") {
                            navigateTo("/refund");
                          }
                          else if (item === "Store location") {
                            navigateTo("/location");
                          }
                          else if (item === "Shipping & return") {
                            navigateTo("/shipping");
                          }
                          else if (item === "Terms & condition") {
                            navigateTo("/terms");
                          }
                        }}
                      ><span>{item}</span></a>
                      ...
                      <div className="pages-menu__item pages-menu__item--nested" key={item.label}>
                        <a
                          ...
                          onClick={(event) => {
                            event.preventDefault();
                            navigateTo(item.label === "About Us" ? "/about-us" : "/contact-us");
                          }}
                        >
                          ...
                          {item.children.map((child) => (
                            <a
                              ...
                              onClick={(event) => {
                                ...
                                else if (child === "About Us 2") {
                                  navigateTo("/about-us-2");
                                }
                                else if (child === "Contact") {
                                  navigateTo("/contact-us");
                                }
                                else if (child === "Contact 2") {
      ...
      <section className="product-cart-page">
        <div className="product-cart-card">
          <div className="product-cart-card__gallery">
            <div className="product-cart-card__main-image">
              <img src={selectedImage} alt={product.title} />
            </div>

            <div className="product-cart-card__thumbs">
          ...
          <div className="product-cart-card__content">
            <span className="product-cart-card__badge">SALE</span>
            <h2>{product.title}</h2>

            <div className="product-cart-card__rating">
              <div className="product-cart-card__stars" aria-hidden="true">
            ...
            <p className="product-cart-card__price">{product.price}</p>
            <p className="product-cart-card__shipping-note">
              Tax included. Shipping calculated at checkout.
            </p>
            <p className="product-cart-card__stock">{product.stock} in stock</p>

            <div className="product-cart-card__divider"></div>

            <p className="product-cart-card__description">{product.description}</p>

            <div className="product-cart-card__options">
              <p>{variantLabel}:</p>
              <div className="product-cart-card__option-list">
            ...
            <div className="product-cart-card__quantity-row">
              <div className="product-cart-card__qty" aria-label={`${product.title} quantity`}>
                <button
                  type="button"
              ...
              <button type="button" className="product-cart-card__add" onClick={handleAddToCart}>
                ADD TO CART
              </button>
            </div>

            <label className="product-cart-card__custom">
            ...
            <button
              type="button"
              className="product-cart-card__continue"
