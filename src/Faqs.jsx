...
const defaultFaqAnswer =
  "We do have restock notifications! If a product is coming back in stock, simply click on the size of the product you're wanting to be notified of and a button should appear that says once somecome back in stock, it will send you an email to notify you";
...
function Faqs() {
  ...
  const [openFaqId, setOpenFaqId] = useState("contact");
  ...
  return (
    <main className="page-shell">
      ...
      <section className="faq-layout">
        <div className="faq-layout__sidebar">
          <p className="faq-layout__eyebrow">MOST COMMON</p>
          {faqCategories.map((item, index) => (
            <button
              key={item}
              type="button"
              className={`faq-layout__category ${index === 0 ? "is-active" : ""}`}
              onClick={() => {
                if (item === "Company policies" || item === "Term and conditions") {
                  navigateToTerms();
                }
        ...
        <div className="faq-layout__content">
          {faqSections.map((section) => (
            <section className="faq-section" key={section.id}>
              <h2>{section.title}</h2>
